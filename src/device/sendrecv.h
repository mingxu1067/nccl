...
struct RunWorkBatch<ncclFuncSendRecv, T, RedOp, NCCL_ALGO_RING, NCCL_PROTO_SIMPLE> {
  ...
  struct A2aMPStep {
    ...
    __device__ __forceinline__ void operator()(int tid, int nthreads, int slice, int maxSliceSize, int nSrcs,
      ...
      static_assert(SlicePerChunk == 1, "A2AMP requires one Simple slice per step");
      const int nelem = min(maxSliceSize, remaining - cursor);
      ncclA2aMPReducePtrs<NPeers>(tid, nthreads, local + cursor, srcPtrs, output + cursor, nelem);
    }
  };

  template <int NPeers>
  struct A2aMPAccumStep {
    const __nv_bfloat16* local;
    float* accum;
    __nv_bfloat16* output;
    int cursor;
    ...
    __device__ __forceinline__ void operator()(int tid, int nthreads, int slice, int maxSliceSize, int nSrcs,
      ...
      if (firstWindow) {
        if (lastWindow)
          ncclA2aMPAccumulatePtrs<NPeers, true, true>(tid, nthreads, local + cursor, srcPtrs, accum + cursor,
                                                         output + cursor, nelem);
        else
          ncclA2aMPAccumulatePtrs<NPeers, true, false>(tid, nthreads, local + cursor, srcPtrs, accum + cursor,
                                                          output + cursor, nelem);
      } else {
        if (lastWindow)
          ncclA2aMPAccumulatePtrs<NPeers, false, true>(tid, nthreads, local + cursor, srcPtrs, accum + cursor,
                                                          output + cursor, nelem);
        else
          ncclA2aMPAccumulatePtrs<NPeers, false, false>(tid, nthreads, local + cursor, srcPtrs, accum + cursor,
                                                           output + cursor, nelem);
      }
    }
  };
  ...
  template <int NPeers>
  __device__ __forceinline__ void runA2aMPPeers(int tid, int nthreads, struct ncclDevWorkP2p* works) {
    ...
    for (int cursor = 0; cursor < partCount; cursor += stepElements) {
      A2aMPStep<NPeers> step{local, output, cursor, partCount, nullptr};
      prims.template process<1, 0>(step);
    }
  }

  template <int NPeers>
  __device__ __forceinline__ void runA2aMPWindow(int tid, int nthreads, struct ncclDevWorkP2p* works,
                                                    const __nv_bfloat16* local, float* accum,
                                                    __nv_bfloat16* output, int partCount, bool firstWindow,
                                                    bool lastWindow) {
    static_assert(0 < NPeers && NPeers <= NCCL_MAX_DIRECT_ARITY, "Unsupported A2AMP peer window");
    ...
    if (tid < sendThreads) {
      ...
      const int stepElements = ncclShmem.comm.p2pChunkSize / sizeof(__nv_bfloat16);
      for (int cursor = 0; cursor < partCount; cursor += stepElements) {
        A2aMPAccumStep<NPeers> step{local, accum, output, cursor, partCount, firstWindow, lastWindow, nullptr};
        prims.template process<1, 0>(step);
      }
    }
  }

  __device__ __forceinline__ void runA2aMPWindows(int tid, int nthreads, struct ncclDevWorkP2p* works,
                                                     int nWorks) {
    const int part = ncclP2pChannelToPart(works[0].nP2pChannels, works[0].channelBase, ncclShmem.channelId);
    size_t byteBegin, byteEnd;
    ncclP2pPartBounds(works[0].nSendChannels, part, works[0].a2aRecvcount * sizeof(__nv_bfloat16), &byteBegin,
    ...
    for (int base = 0; base < nWorks; base += NCCL_MAX_DIRECT_ARITY) {
      ...
      switch (peers) {
        case 1: runA2aMPWindow<1>(tid, nthreads, works + base, local, accum, output, partCount, firstWindow,
                                     lastWindow); break;
        case 2: runA2aMPWindow<2>(tid, nthreads, works + base, local, accum, output, partCount, firstWindow,
                                     lastWindow); break;
        case 3: runA2aMPWindow<3>(tid, nthreads, works + base, local, accum, output, partCount, firstWindow,
                                     lastWindow); break;
        case 4: runA2aMPWindow<4>(tid, nthreads, works + base, local, accum, output, partCount, firstWindow,
                                     lastWindow); break;
        case 5: runA2aMPWindow<5>(tid, nthreads, works + base, local, accum, output, partCount, firstWindow,
                                     lastWindow); break;
        case 6: runA2aMPWindow<6>(tid, nthreads, works + base, local, accum, output, partCount, firstWindow,
                                     lastWindow); break;
        case 7: runA2aMPWindow<7>(tid, nthreads, works + base, local, accum, output, partCount, firstWindow,
                                     lastWindow); break;
      }
      __syncthreads();
    }
  ...
  __device__ __forceinline__ void runA2aMP(int tid, int nthreads, struct ncclDevWorkP2p* works, int nWorks) {
    if (nWorks > NCCL_MAX_DIRECT_ARITY) return runA2aMPWindows(tid, nthreads, works, nWorks);
    switch (nWorks) {
      case 1: return runA2aMPPeers<1>(tid, nthreads, works);
      case 2: return runA2aMPPeers<2>(tid, nthreads, works);
      case 3: return runA2aMPPeers<3>(tid, nthreads, works);
      case 4: return runA2aMPPeers<4>(tid, nthreads, works);
      case 5: return runA2aMPPeers<5>(tid, nthreads, works);
      case 6: return runA2aMPPeers<6>(tid, nthreads, works);
      case 7: return runA2aMPPeers<7>(tid, nthreads, works);
    }
  }
  ...
  __device__ __forceinline__ void run() {
    ...
    if (works[0].a2aMP && nWorks > NCCL_MAX_DIRECT_ARITY) {
      // A2AMP extension batches may contain as many as 127 descriptors.
