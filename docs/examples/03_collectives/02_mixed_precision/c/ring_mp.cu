...
int main(int argc, char** argv) {
  ...
  __nv_bfloat16 *send, *allReduceOutput, *reduceScatterOutput;
  ...
  // RingMP communicates and returns BF16 while accumulating reduction values in FP32.
  NCCL_CHECK(ncclAllReduceRingMP(send, allReduceOutput, nullptr, 0, count, ncclSum, comm, 0));
  NCCL_CHECK(ncclReduceScatterRingMP(send, reduceScatterOutput, nullptr, 0, recvcount, ncclSum, comm, 0));
  ...
  if (rank == 0)
    std::printf("RingMP AllReduce and ReduceScatter: %s\n", allCorrect ? "PASS" : "FAIL");
